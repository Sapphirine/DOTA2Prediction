...
def get_all_items_list():
    with open('dictionaries/items.csv', 'rb') as f:
        reader = csv.reader(f)
        items_list = map(tuple, reader)
    return items_list


def extract_match_features(match, items_list, time_point=None):

    extract_items_time = items_list
    extract_items_count = items_list
    ...
    for player_index, player in enumerate(match['players']):
        ...
        feats += [
            ...
            (player_id + '_buyback', len(filter_events(player['buyback_log'], time_point))),
            (player_id + '_deaths', len([
                1 for other_player in match['players']
...
def iterate_matches(matches_filename):
    with bz2.BZ2File(matches_filename) as f:
        for n, line in enumerate(f):
            ...
            print '='*100
            match = json.loads(line.decode('utf8'))
            yield match
            if (n + 1) % 1000 == 0:
...
def create_table(matches_filename, time_point):
    df = {}
    fields = None
    items_list = get_all_items_list()
    for match in iterate_matches(matches_filename):
        match
    #     features = extract_match_features(match, items_list, time_point)
    #     if fields is None:
